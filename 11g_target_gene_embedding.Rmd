...
complexes_in_function <- list(
  "Regulation of transcription" = sort(c("CCR4-NOT complex", "RNA N6-methyladenosine methyltransferase complex", "INO80 chromatin remodeling complex", "NuA4/Tip60-HAT complex")),
  "Transcription" = sort(c("RNA Polymerase II", "iPSC maintennance", "Paf complex", "Mediator complex", "TFIID", "TFIIH")),
  "RNA processing" = sort(c("Spliceosome", '26S proteasome')), #, "Cleavage and polyadenylation factor")),
  "Peptide synthesis" = sort(c("EIF2 complex", 'tRNA ligases', "Mitochondrial ribosome", "TOMM40 complex")),
  'Translation and Post-translational processing' = sort(c('Integrator complex', "EIF3 complex", 'TREX complex', 'Exosome'))
)
function_colors <- hue_pal()(length(complexes_in_function)); names(function_colors) <- names(complexes_in_function)
complexes_in_function_df <- data.frame(
  complex_function = rep(names(complexes_in_function), unlist(lapply(complexes_in_function, length))),
  complex_name = unlist(complexes_in_function)
) %>%
  mutate(function_color = function_colors[match(complex_function, names(function_colors))])

## complexes within functions
complex_anno_list <- list(
  "INO80 chromatin remodeling complex" = c('ACTR8', 'INO80', 'INO80B', 'NFRKB'),
  'RNA N6-methyladenosine methyltransferase complex' = c('RBM15', 'YTHDF12', 'CBLL1', 'METTL3', 'METTL14', 'ZC3H13', 'SERPINB1'),
  ...
  'iPSC maintennance' = c("POU5F1", "NANOG", "PRDM14"),
  'Mediator complex' = grep(row.names(df4heatmap), pattern = '^MED', value = T),
  'RNA Polymerase II' = grep(row.names(df4heatmap), pattern = '^POLR2', value = T),
  'TFIID' = grep(row.names(df4heatmap), pattern = '^TAF', value = T),
  'Spliceosome' = grep(target_anno %>% dplyr::filter(cluster_function == "RNA processing") %>% .$gene, pattern = "^CPSF", value = T, invert = T),
  "EIF2 complex" = grep(row.names(df4heatmap), pattern = '^EIF2', value = T),
  "tRNA ligases" = c('CARS', 'FARSB', 'GARS', 'RARS', 'SARS', 'TARS2'),
  "TOMM40 complex" = grep(row.names(df4heatmap), pattern = 'TOMM', value = T),
  "Mitochondrial ribosome" = grep(row.names(df4heatmap), pattern = '^MRP', value = T),
  'Exosome' = grep(row.names(df4heatmap), pattern = '^EXOSC', value = T),
  'TREX complex' = grep(row.names(df4heatmap), pattern = '^THOC', value = T),
  "EIF3 complex" = grep(row.names(df4heatmap), pattern = '^EIF3', value = T),
  "26S proteasome" = grep(row.names(df4heatmap), pattern = '^PSM', value = T),
  "Integrator complex" = c("INTS8", "INTS1", "INTS2", "INTS5")
)
complex_colors <- rep('', length(complex_anno_list))
names(complex_colors) <- names(complex_anno_list)
for (n_ind in 1:length(names(complexes_in_function))){
  n <- names(complexes_in_function)[n_ind]
  cs <- complexes_in_function[[n]]
  base_color_angle <- 15 + 360*(n_ind-1)/length(complexes_in_function)
  for (c_ind in 1:length(cs)){
    complex_colors[cs[c_ind]] <- hue_pal(h = rep(base_color_angle, 2), l = (length(cs) - c_ind + 1)/length(cs)*80 + 10)(1)
  }
}

genes_in_complexes_df <- data.frame(
  gene = unlist(complex_anno_list),
  complex_name = rep(names(complex_anno_list), unlist(lapply(complex_anno_list, length)))
) %>% 
  mutate(complex_color = complex_colors[match(complex_name, names(complex_colors))])

## color scheme
color_scheme <- left_join(left_join(data.frame(gene = row.names(df4heatmap)),
                          genes_in_complexes_df),
                          complexes_in_function_df)


## annotations on left
cell_dim_points <- 2.25
fig_height_inches <- 10; fig_height_points <- fig_height_inches/72
fig_width_inches <- 17
heatmap_height_points <- dim(df4heatmap)[1]*cell_dim_points; heatmap_height_inches <- heatmap_height_points/72
y_scale <- ((fig_height_inches - (fig_height_inches - heatmap_height_inches)/2)/fig_height_inches -
              (fig_height_inches - heatmap_height_inches)/2/fig_height_inches)
y_shift <- (fig_height_inches - heatmap_height_inches)/2/fig_height_inches
x_shift <- (fig_width_inches - heatmap_height_inches)/2/fig_width_inches
row_label_df <- target_anno[match(order4heatmap, target_anno$gene),] %>%
  ...
  summarize(y_mean = 1 - mean(heatmap_order)/dim(target_anno)[1]) %>%
  mutate(y_mean = y_mean*y_scale +y_shift)
...
row_anno <- target_anno %>% 
  dplyr::select(c('gene', 'cluster_function')) %>%
  mutate(Complex = color_scheme$complex_name[match(gene, color_scheme$gene)]) %>%
  column_to_rownames('gene')
colnames(row_anno) <- c("Function", "Complex")
row_anno <- dplyr::select(row_anno, c("Complex", "Function"))
...
p <- my_heatmap(df4heatmap_lowertri,
                ...
                annotation_row = row_anno,
                annotation_colors = list(Function = complex_cluster_colors,
                                         Complex = complex_colors),
                cellheight = cell_dim_points, cellwidth = cell_dim_points,
                border_col = NA, na_col = "white")
p <- ggarrange(p[[4]])
p + 
  annotate(geom = 'richtext', label = row_label_df$cluster_labels, 
           x = x_shift/3, y = row_label_df$y_mean, 
...
```{r final_heatmap_labeled, fig.width = 50, fig.height = 50, echo = F}
p <- my_heatmap(df4heatmap_lowertri,
                treeheight_row = 0, treeheight_col = 0,
                min_c = -0.8, max_c = 0.8,
                #show_rownames = F, show_colnames = F,
